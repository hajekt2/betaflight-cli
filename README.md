...
Camera control is exposed as a small action family for analog camera menu key presses.
...
`camera keys` lists the five upstream camera-control keys without connecting to hardware.
`camera press KEY` returns a plan by default and only sends `MSP_CAMERA_CONTROL` when `--yes` is supplied.
Camera key presses are immediate actions and do not require `save`.
...
`receiver status` reads receiver configuration, channel map, configured RSSI channel, active TX RSSI source and RTC status, RC deadband, RX failsafe rows, and live RC channels over MSP.
...
`motors status` reads motor configuration, live motor outputs, modern motor telemetry, legacy ESC sensor temperature/RPM data, 3D motor config, and output order over MSP.
