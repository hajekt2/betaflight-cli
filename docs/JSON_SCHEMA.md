...
Successful apply responses set `applied` and `stopped`, include read-only runtime preflight state, include a read-only post-stop motor snapshot, include command response lines, and emit a `motor_output` side effect.
