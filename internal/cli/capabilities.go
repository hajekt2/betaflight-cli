...
func coverageDomains(commandSet map[string]bool) []coverageDomain {
	domains := []coverageDomain{
		...
		implementedDomain("receiver", commandSet, []string{"betaflight-cli receiver list", "betaflight-cli receiver status", "betaflight-cli rxrange list"}, []string{"betaflight-cli receiver rxfail", "betaflight-cli receiver set-config-json", "betaflight-cli receiver set-rxfail", "betaflight-cli receiver set-rxfail-json", "betaflight-cli receiver set-rssi-channel", "betaflight-cli receiver set-map", "betaflight-cli receiver set-map-json", "betaflight-cli receiver set-deadband", "betaflight-cli receiver set-deadband-json", "betaflight-cli rxrange set"}, nil, []string{"receiver", "receiver_config", "rxrange", "change_plan", "rx_fail", "rx_fail_table", "rssi_channel", "rc_map", "rc_deadband"}, "Receiver config, RC channels, RX failsafe rows, RSSI channel, RC map, RC deadband, and channel ranges are covered; receiver config, RX failsafe, RC map, and RC deadband writes have confirmed typed MSP paths."),
		...
		implementedDomain("battery-failsafe", commandSet, []string{"betaflight-cli battery list", "betaflight-cli battery status", "betaflight-cli failsafe list", "betaflight-cli failsafe status"}, []string{"betaflight-cli battery set", "betaflight-cli battery set-config-json", "betaflight-cli battery set-profile-json", "betaflight-cli battery set-voltage-meter", "betaflight-cli battery set-current-meter", "betaflight-cli failsafe set", "betaflight-cli failsafe set-arming-json", "betaflight-cli failsafe set-config-json", "betaflight-cli failsafe set-board-alignment"}, nil, []string{"battery", "battery_config", "battery_profile", "failsafe", "arming_config", "failsafe_config", "change_plan", "board_alignment", "voltage_meter_config", "current_meter_config"}, "Battery and failsafe settings plus arming/failsafe status are covered; battery config, battery profile, voltage/current meter calibration, arming config, failsafe config, and board alignment have confirmed typed MSP writes."),
		implementedDomain("vtx-osd-leds", commandSet, []string{"betaflight-cli vtx config", "betaflight-cli vtx list", "betaflight-cli osd list", "betaflight-cli osd status", "betaflight-cli vtxtable list", "betaflight-cli leds list", "betaflight-cli leds status"}, []string{"betaflight-cli vtx set", "betaflight-cli vtx set-config", "betaflight-cli osd set", "betaflight-cli osd set-canvas", "betaflight-cli osd set-general-json", "betaflight-cli osd set-video-system", "betaflight-cli osd set-position", "betaflight-cli osd set-stat", "betaflight-cli osd set-timer", "betaflight-cli vtxtable set", "betaflight-cli vtxtable set-json", "betaflight-cli vtxtable set-band", "betaflight-cli vtxtable set-power", "betaflight-cli leds set", "betaflight-cli leds set-colors-json", "betaflight-cli leds set-mode-color", "betaflight-cli leds set-values"}, nil, []string{"vtx", "vtx_config", "osd", "osd_canvas", "osd_general_config", "osd_video_system", "osd_position", "osd_stat", "osd_timer", "vtxtable", "vtxtable_band", "vtxtable_power", "leds", "change_plan", "led_colors", "led_mode_color", "led_values"}, "VTX, OSD, VTX table, and LED strip surfaces are implemented without graphical layout editing; VTX config, OSD canvas, OSD general config, OSD video system, OSD item/stat/timer changes, VTX table rows, full VTX table JSON writes, LED color tables, LED mode colors, and LED value tuning have confirmed typed MSP writes."),
		implementedDomain("motors-servos-mixer", commandSet, []string{"betaflight-cli mixer status", "betaflight-cli motors status", "betaflight-cli motors test-plan", "betaflight-cli servos list", "betaflight-cli servos status", "betaflight-cli adjustments list", "betaflight-cli adjustments status"}, []string{"betaflight-cli mixer set-config-json", "betaflight-cli motors set-config", "betaflight-cli motors set-3d-config", "betaflight-cli servos set", "betaflight-cli servos reverse", "betaflight-cli servos set-json", "betaflight-cli servos set-config", "betaflight-cli servos set-mix-rule", "betaflight-cli adjustments set", "betaflight-cli adjustments set-json", "betaflight-cli adjustments set-range"}, []string{"betaflight-cli motors test-apply"}, []string{"mixer", "mixer_config", "motors", "motor_config", "motor_3d_config", "motor_test_plan", "servos", "servo_table", "servo_config", "servo_mix_rule", "adjustments", "adjustment_table", "adjustment_range", "change_plan"}, "Mixer, motor, and servo reads are implemented; mixer configuration, motor configuration, 3D motor configuration, servo table JSON, servo row, servo mix rule, adjustment range JSON, and adjustment range changes have confirmed typed MSP writes; motor output testing is bounded, dangerous, and confirmation-gated."),
