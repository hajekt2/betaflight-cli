...
func coverageDomains(commandSet map[string]bool) []coverageDomain {
	domains := []coverageDomain{
		...
		implementedDomain("ports-and-modes", commandSet, []string{"betaflight-cli serial list", "betaflight-cli serial status", "betaflight-cli modes list", "betaflight-cli modes active"}, []string{"betaflight-cli serial set", "betaflight-cli modes set", "betaflight-cli modes set-range"}, nil, []string{"serial", "modes", "mode_range", "change_plan"}, "Serial rows and AUX modes have read, typed status, and plan/apply surfaces; AUX mode range writes also have a confirmed typed MSP path."),
