...
func coverageDomains(commandSet map[string]bool) []coverageDomain {
	domains := []coverageDomain{
		...
		implementedDomain("sensor-maintenance", commandSet, []string{"betaflight-cli sensors status"}, []string{"betaflight-cli sensors set-config", "betaflight-cli sensors set-compass-declination"}, []string{"betaflight-cli sensors calibrate-accelerometer", "betaflight-cli sensors calibrate-magnetometer"}, []string{"sensors", "sensor_calibration", "sensor_config", "compass_config"}, "Sensor status, hardware selection, and compass declination writes are exposed through typed MSP commands; accelerometer/magnetometer calibration actions remain dangerous and confirmation-gated."),
