...
func coverageDomains(commandSet map[string]bool) []coverageDomain {
	domains := []coverageDomain{
		...
		implementedDomain("camera-control", commandSet, []string{"betaflight-cli camera keys"}, []string{"betaflight-cli camera press"}, nil, []string{"camera_keys", "camera_control"}, "Analog camera-control key actions are exposed through confirmed MSP_CAMERA_CONTROL with plan-first output and explicit confirmation before sending a key press."),
		...
		implementedDomain("arming-safety", commandSet, []string{"betaflight-cli arming status"}, []string{"betaflight-cli arming lock", "betaflight-cli arming unlock"}, []string{"betaflight-cli rx-bind start"}, []string{"arming_lock", "rx_bind"}, "Arming-lock state reads and MSP-managed arming gate toggles are exposed with explicit confirmation; RX bind pulses are dangerous and confirmation-gated."),
		implementedDomain("navigation-waypoints", commandSet, []string{"betaflight-cli wp list", "betaflight-cli wp get"}, []string{"betaflight-cli wp set-json"}, []string{"betaflight-cli wp clear"}, []string{"waypoints", "waypoint", "change_plan"}, "Waypoint missions are read through the firmware CLI waypoint surface; writes are plan-first waypoint insert lines with explicit apply, and clears are dangerous."),
		implementedDomain("motor-reordering", commandSet, []string{"betaflight-cli motor-reorder show"}, []string{"betaflight-cli motor-reorder set"}, nil, []string{"motor_output_reordering", "motor_reorder"}, "Motor output reordering has a typed MSP v2 read-back and confirmed full-order writes validated against the supported motor count."),
		implementedDomain("serial-config-v2", commandSet, nil, []string{"betaflight-cli serial apply-config-v2-json"}, nil, []string{"common_serial_config"}, "The MSP v2 common serial config write path complements the legacy serial table writes for firmware that prefers the common namespace."),
		implementedDomain("osd-characters-video", commandSet, []string{"betaflight-cli osd char-get", "betaflight-cli osd video-config"}, []string{"betaflight-cli osd char-set", "betaflight-cli osd set-video-config-json"}, nil, []string{"osd_char", "osd_video_config"}, "OSD font character upload is typed against MSP_OSD_CHAR_WRITE; character and video-config reads fail with ok:false when the target does not implement those external-OSD codes."),
		implementedDomain("blackbox-analysis", commandSet, []string{"betaflight-cli blackbox analyze"}, nil, nil, []string{"analysis"}, "Offline Blackbox analysis computes gyro noise spectra, step-response metrics, and motor balance without hardware."),
		implementedDomain("live-monitoring", commandSet, []string{"betaflight-cli watch"}, nil, nil, []string{"sample"}, "Watch mode streams NDJSON runtime envelopes over one held connection at a bounded interval."),
		implementedDomain("factory-reset", commandSet, nil, nil, []string{"betaflight-cli configuration reset"}, []string{"change_plan"}, "Factory reset applies defaults nosave through the reviewed plan pipeline as a dangerous action with an explicit save decision."),
	}
...
func capabilityMetadataRegistry() map[string]capabilityMetadata {
	...
	registry := map[string]capabilityMetadata{
		...
		"betaflight-cli camera keys":                     {RequiresConnection: false, Operation: "offline", Confirmation: "none", OutputRoot: "camera_keys", Tags: []string{"camera", "offline"}},
		"betaflight-cli camera press":                    {RequiresConnection: true, Operation: "plan_or_write", Confirmation: "--yes", OutputRoot: "camera_control", Tags: []string{"camera", "write", "msp"}},
		...
		"betaflight-cli arming status":                     {RequiresConnection: true, Operation: "read_only", Confirmation: "none", OutputRoot: "arming_lock", Tags: []string{"arming", "read"}},
		"betaflight-cli arming lock":                       {RequiresConnection: true, Operation: "dangerous", Confirmation: "--yes; disarms immediately if armed", OutputRoot: "arming_lock", Tags: []string{"arming", "dangerous"}},
		"betaflight-cli arming unlock":                     {RequiresConnection: true, Operation: "dangerous", Confirmation: "--yes", OutputRoot: "arming_lock", Tags: []string{"arming", "dangerous"}},
		"betaflight-cli wp list":                           {RequiresConnection: true, Operation: "read_only", Confirmation: "none", OutputRoot: "waypoints", Tags: []string{"waypoints", "read"}},
		"betaflight-cli wp get":                            {RequiresConnection: true, Operation: "read_only", Confirmation: "none", OutputRoot: "waypoint", Tags: []string{"waypoints", "read"}},
		"betaflight-cli wp set-json":                       {RequiresConnection: true, Operation: "plan_or_write", Confirmation: "--yes with --apply or --save", OutputRoot: "change_plan", Input: "JSON array or object with waypoints", Tags: []string{"waypoints", "write", "plan"}},
		"betaflight-cli wp clear":                          {RequiresConnection: true, Operation: "dangerous", Confirmation: "--yes with --apply", OutputRoot: "change_plan", Tags: []string{"waypoints", "dangerous"}},
		"betaflight-cli motor-reorder show":                {RequiresConnection: true, Operation: "read_only", Confirmation: "none", OutputRoot: "motor_output_reordering", Tags: []string{"motors", "read"}},
		"betaflight-cli motor-reorder set":                 {RequiresConnection: true, Operation: "write", Confirmation: "--yes", OutputRoot: "motor_reorder", Input: "JSON object with order array", Tags: []string{"motors", "write"}},
		"betaflight-cli rx-bind start":                     {RequiresConnection: true, Operation: "dangerous", Confirmation: "--yes", OutputRoot: "rx_bind", Tags: []string{"receiver", "dangerous", "bind"}},
		"betaflight-cli watch":                             {RequiresConnection: true, Operation: "read_only", Confirmation: "none", OutputRoot: "sample", Input: "profile: telemetry, status, motors, or gps; flags --interval, --count, --duration emit NDJSON envelopes", Tags: []string{"watch", "streaming", "read"}},
		"betaflight-cli configuration reset":               {RequiresConnection: true, Operation: "dangerous", Confirmation: "--apply and --yes", OutputRoot: "change_plan", Tags: []string{"configuration", "dangerous", "reset"}},
		"betaflight-cli serial apply-config-v2-json":       {RequiresConnection: true, Operation: "write", Confirmation: "--yes", OutputRoot: "common_serial_config", Input: "JSON array or object with ports", Tags: []string{"serial", "write", "msp_v2"}},
		"betaflight-cli blackbox analyze":                  {RequiresConnection: false, Operation: "offline", Confirmation: "none", OutputRoot: "analysis", Input: "Blackbox log file or stdin; --log-index N reads onboard dataflash and requires a connection", Tags: []string{"blackbox", "offline", "analysis"}},
		"betaflight-cli osd char-get":                      {RequiresConnection: true, Operation: "read_only", Confirmation: "none", OutputRoot: "osd_char", Tags: []string{"osd", "read"}},
		"betaflight-cli osd char-set":                      {RequiresConnection: true, Operation: "write", Confirmation: "--yes", OutputRoot: "osd_char", Input: "JSON object with index and bitmap or rows", Tags: []string{"osd", "write"}},
		"betaflight-cli osd video-config":                  {RequiresConnection: true, Operation: "read_only", Confirmation: "none", OutputRoot: "osd_video_config", Tags: []string{"osd", "read"}},
		"betaflight-cli osd set-video-config-json":         {RequiresConnection: true, Operation: "write", Confirmation: "--yes", OutputRoot: "osd_video_config", Input: "JSON object with video_system and units", Tags: []string{"osd", "write"}},
