...
func coverageDomains(commandSet map[string]bool) []coverageDomain {
	domains := []coverageDomain{
		...
		implementedDomain("vtx-osd-leds", commandSet, []string{"betaflight-cli vtx config", "betaflight-cli vtx list", "betaflight-cli osd list", "betaflight-cli osd status", "betaflight-cli vtxtable list", "betaflight-cli leds list", "betaflight-cli leds status"}, []string{"betaflight-cli vtx set", "betaflight-cli vtx set-config", "betaflight-cli osd set", "betaflight-cli osd set-canvas", "betaflight-cli vtxtable set", "betaflight-cli vtxtable set-band", "betaflight-cli vtxtable set-power", "betaflight-cli leds set", "betaflight-cli leds set-values"}, nil, []string{"vtx", "vtx_config", "osd", "osd_canvas", "vtxtable", "vtxtable_band", "vtxtable_power", "leds", "change_plan", "led_values"}, "VTX, OSD, VTX table, and LED strip surfaces are implemented without graphical layout editing; VTX config, OSD canvas, VTX table rows, and LED value tuning have confirmed typed MSP writes."),
