...
func coverageDomains(commandSet map[string]bool) []coverageDomain {
	domains := []coverageDomain{
		...
		implementedDomain("sensor-maintenance", commandSet, []string{"betaflight-cli sensors status"}, []string{"betaflight-cli sensors set-compass-declination"}, []string{"betaflight-cli sensors calibrate-accelerometer", "betaflight-cli sensors calibrate-magnetometer"}, []string{"sensors", "sensor_calibration", "compass_config"}, "Sensor status and compass declination writes are exposed through typed MSP commands; accelerometer/magnetometer calibration actions remain dangerous and confirmation-gated."),
		implementedDomain("receiver", commandSet, []string{"betaflight-cli receiver list", "betaflight-cli receiver status", "betaflight-cli rxrange list"}, []string{"betaflight-cli receiver rxfail", "betaflight-cli receiver set-rssi-channel", "betaflight-cli receiver set-deadband", "betaflight-cli rxrange set"}, nil, []string{"receiver", "rxrange", "change_plan", "rssi_channel", "rc_deadband"}, "Receiver config, RC channels, RX failsafe rows, RSSI channel, RC deadband, and channel ranges are covered."),
		...
		implementedDomain("gps", commandSet, []string{"betaflight-cli gps list", "betaflight-cli gps status"}, []string{"betaflight-cli gps set", "betaflight-cli gps set-config"}, nil, []string{"gps", "change_plan", "gps_config"}, "GPS config, position, rescue settings, PID terms, and satellite info are covered when firmware supplies them; provider configuration has a confirmed typed MSP write."),
		...
		implementedDomain("motors-servos-mixer", commandSet, []string{"betaflight-cli mixer status", "betaflight-cli motors status", "betaflight-cli motors test-plan", "betaflight-cli servos list", "betaflight-cli servos status", "betaflight-cli adjustments list", "betaflight-cli adjustments status"}, []string{"betaflight-cli motors set-3d-config", "betaflight-cli servos set", "betaflight-cli servos reverse", "betaflight-cli adjustments set"}, []string{"betaflight-cli motors test-apply"}, []string{"mixer", "motors", "motor_3d_config", "motor_test_plan", "servos", "adjustments", "change_plan"}, "Motor and servo reads are implemented; 3D motor configuration has a confirmed typed MSP write; motor output testing is bounded, dangerous, and confirmation-gated."),
