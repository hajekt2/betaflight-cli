...
func coverageDomains(commandSet map[string]bool) []coverageDomain {
	domains := []coverageDomain{
		...
		implementedDomain("receiver", commandSet, []string{"betaflight-cli receiver list", "betaflight-cli receiver status", "betaflight-cli rxrange list"}, []string{"betaflight-cli receiver rxfail", "betaflight-cli receiver set-rssi-channel", "betaflight-cli rxrange set"}, nil, []string{"receiver", "rxrange", "change_plan", "rssi_channel"}, "Receiver config, RC channels, RX failsafe rows, RSSI channel, and channel ranges are covered."),
