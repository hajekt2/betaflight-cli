...
func coverageDomains(commandSet map[string]bool) []coverageDomain {
	domains := []coverageDomain{
		...
		implementedDomain("vtx-osd-leds", commandSet, []string{"betaflight-cli vtx config", "betaflight-cli vtx list", "betaflight-cli osd list", "betaflight-cli osd status", "betaflight-cli vtxtable list", "betaflight-cli leds list", "betaflight-cli leds status"}, []string{"betaflight-cli vtx set", "betaflight-cli vtx set-config", "betaflight-cli osd set", "betaflight-cli osd set-canvas", "betaflight-cli osd set-position", "betaflight-cli osd set-stat", "betaflight-cli osd set-timer", "betaflight-cli vtxtable set", "betaflight-cli vtxtable set-band", "betaflight-cli vtxtable set-power", "betaflight-cli leds set", "betaflight-cli leds set-colors-json", "betaflight-cli leds set-mode-color", "betaflight-cli leds set-values"}, nil, []string{"vtx", "vtx_config", "osd", "osd_canvas", "osd_position", "osd_stat", "osd_timer", "vtxtable", "vtxtable_band", "vtxtable_power", "leds", "change_plan", "led_colors", "led_mode_color", "led_values"}, "VTX, OSD, VTX table, and LED strip surfaces are implemented without graphical layout editing; VTX config, OSD canvas, OSD item/stat/timer changes, VTX table rows, LED color tables, LED mode colors, and LED value tuning have confirmed typed MSP writes."),
