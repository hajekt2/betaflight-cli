...
func coverageDomains(commandSet map[string]bool) []coverageDomain {
	domains := []coverageDomain{
		...
		implementedDomain("storage-blackbox", commandSet, []string{"betaflight-cli storage status", "betaflight-cli storage export", "betaflight-cli blackbox config", "betaflight-cli blackbox inspect"}, nil, []string{"betaflight-cli storage erase"}, []string{"storage", "dataflash_export", "storage_erase", "blackbox", "inspection"}, "Storage summaries, Dataflash export, Dataflash erase, Blackbox configuration, and initial offline log inspection are implemented."),
