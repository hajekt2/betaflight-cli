...
func coverageDomains(commandSet map[string]bool) []coverageDomain {
	domains := []coverageDomain{
		...
		implementedDomain("configuration-restore", commandSet, []string{"betaflight-cli configuration validate", "betaflight-cli restore plan", "betaflight-cli presets plan", "betaflight-cli batch plan"}, []string{"betaflight-cli restore apply", "betaflight-cli presets apply", "betaflight-cli presets fetch", "betaflight-cli batch apply"}, []string{"betaflight-cli save"}, []string{"configuration_validation", "change_plan"}, "Plan/apply/save is implemented with explicit confirmation and defaults safeguards."),
...
func capabilityMetadataRegistry() map[string]capabilityMetadata {
	...
	registry := map[string]capabilityMetadata{
		...
		"betaflight-cli configuration apply": {
			...
			Operation:          "plan_or_write",
			Confirmation:       "--yes with --include-defaults or --save",
			OutputRoot:         "change_plan",
			Tags:               []string{"configuration", "write"},
		},
