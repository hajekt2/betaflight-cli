...
func coverageDomains(commandSet map[string]bool) []coverageDomain {
	domains := []coverageDomain{
		...
		implementedDomain("beeper-transponder", commandSet, []string{"betaflight-cli beeper config", "betaflight-cli transponder config"}, []string{"betaflight-cli beeper enable", "betaflight-cli beeper disable"}, nil, []string{"beeper", "transponder"}, "Beeper and transponder configuration reads are covered and beeper toggles are available through CLI-backed write plans."),
