...
func coverageDomains(commandSet map[string]bool) []coverageDomain {
	domains := []coverageDomain{
		...
		implementedDomain("sensor-maintenance", commandSet, []string{"betaflight-cli sensors status"}, nil, []string{"betaflight-cli sensors calibrate-accelerometer", "betaflight-cli sensors calibrate-magnetometer"}, []string{"sensors", "sensor_calibration"}, "Sensor status and accelerometer/magnetometer calibration actions are exposed through typed MSP commands."),
