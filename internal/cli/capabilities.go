...
func coverageDomains(commandSet map[string]bool) []coverageDomain {
	domains := []coverageDomain{
		...
		implementedDomain("pid-rates-filters", commandSet, []string{"betaflight-cli pid list", "betaflight-cli pid status", "betaflight-cli rates list", "betaflight-cli rates status", "betaflight-cli filters list", "betaflight-cli filters status"}, []string{"betaflight-cli pid set", "betaflight-cli pid set-gains-json", "betaflight-cli rates set", "betaflight-cli rates set-profile-json", "betaflight-cli filters set", "betaflight-cli filters set-advanced-json"}, nil, []string{"pid", "pid_gains", "rates", "rate_profile", "filters", "advanced_config", "change_plan"}, "Core tuning domains have metadata-backed setting lists, typed status, and setting plans; PID gain, rate profile, and advanced loop/motor config writes have confirmed typed MSP paths."),
