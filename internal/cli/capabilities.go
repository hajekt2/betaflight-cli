...
func coverageDomains(commandSet map[string]bool) []coverageDomain {
	domains := []coverageDomain{
		...
		implementedDomain("motors-servos-mixer", commandSet, []string{"betaflight-cli mixer status", "betaflight-cli motors status", "betaflight-cli motors test-plan", "betaflight-cli servos list", "betaflight-cli servos status", "betaflight-cli adjustments list", "betaflight-cli adjustments status"}, []string{"betaflight-cli motors set-config", "betaflight-cli motors set-3d-config", "betaflight-cli servos set", "betaflight-cli servos reverse", "betaflight-cli adjustments set"}, []string{"betaflight-cli motors test-apply"}, []string{"mixer", "motors", "motor_config", "motor_3d_config", "motor_test_plan", "servos", "adjustments", "change_plan"}, "Motor and servo reads are implemented; motor configuration and 3D motor configuration have confirmed typed MSP writes; motor output testing is bounded, dangerous, and confirmation-gated."),
