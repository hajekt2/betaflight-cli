...
func coverageDomains(commandSet map[string]bool) []coverageDomain {
	domains := []coverageDomain{
		...
		implementedDomain("settings", commandSet, []string{"betaflight-cli settings list", "betaflight-cli settings get", "betaflight-cli settings diff"}, []string{"betaflight-cli settings set", "betaflight-cli settings set-json"}, nil, []string{"settings", "change_plan"}, "Core setting inventory, targeted reads, and single or multi-setting plan-aware changes are implemented with dry-run diffs before write."),
...
func capabilityMetadataRegistry() map[string]capabilityMetadata {
	...
	registry := map[string]capabilityMetadata{
		...
		"betaflight-cli features set-json":               {RequiresConnection: true, Operation: "write", Confirmation: "--yes only with --apply", OutputRoot: "change_plan", Input: "JSON object with enable/enabled and disable/disabled feature name arrays", Tags: []string{"features", "write", "plan"}},
