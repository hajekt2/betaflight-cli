...
func coverageDomains(commandSet map[string]bool) []coverageDomain {
	domains := []coverageDomain{
		...
		implementedDomain("pid-rates-filters", commandSet, []string{"betaflight-cli pid list", "betaflight-cli pid status", "betaflight-cli rates list", "betaflight-cli rates status", "betaflight-cli filters list", "betaflight-cli filters status"}, []string{"betaflight-cli pid set", "betaflight-cli pid set-gains-json", "betaflight-cli pid set-advanced-json", "betaflight-cli rates set", "betaflight-cli rates set-profile-json", "betaflight-cli filters set", "betaflight-cli filters set-advanced-json", "betaflight-cli filters set-filter-json"}, nil, []string{"pid", "pid_gains", "pid_advanced", "rates", "rate_profile", "filters", "advanced_config", "filter_config", "change_plan"}, "Core tuning domains have metadata-backed setting lists, typed status, and setting plans; PID gain, PID advanced profile, rate profile, advanced loop/motor config, and filter config writes have confirmed typed MSP paths."),
		...
		implementedDomain("vtx-osd-leds", commandSet, []string{"betaflight-cli vtx config", "betaflight-cli vtx list", "betaflight-cli osd list", "betaflight-cli osd status", "betaflight-cli vtxtable list", "betaflight-cli leds list", "betaflight-cli leds status"}, []string{"betaflight-cli vtx set", "betaflight-cli vtx set-config", "betaflight-cli osd set", "betaflight-cli osd set-canvas", "betaflight-cli osd set-position", "betaflight-cli osd set-stat", "betaflight-cli osd set-timer", "betaflight-cli vtxtable set", "betaflight-cli vtxtable set-band", "betaflight-cli vtxtable set-power", "betaflight-cli leds set", "betaflight-cli leds set-values"}, nil, []string{"vtx", "vtx_config", "osd", "osd_canvas", "osd_position", "osd_stat", "osd_timer", "vtxtable", "vtxtable_band", "vtxtable_power", "leds", "change_plan", "led_values"}, "VTX, OSD, VTX table, and LED strip surfaces are implemented without graphical layout editing; VTX config, OSD canvas, OSD item/stat/timer changes, VTX table rows, and LED value tuning have confirmed typed MSP writes."),
		implementedDomain("motors-servos-mixer", commandSet, []string{"betaflight-cli mixer status", "betaflight-cli motors status", "betaflight-cli motors test-plan", "betaflight-cli servos list", "betaflight-cli servos status", "betaflight-cli adjustments list", "betaflight-cli adjustments status"}, []string{"betaflight-cli mixer set-config-json", "betaflight-cli motors set-config", "betaflight-cli motors set-3d-config", "betaflight-cli servos set", "betaflight-cli servos reverse", "betaflight-cli servos set-config", "betaflight-cli servos set-mix-rule", "betaflight-cli adjustments set", "betaflight-cli adjustments set-range"}, []string{"betaflight-cli motors test-apply"}, []string{"mixer", "mixer_config", "motors", "motor_config", "motor_3d_config", "motor_test_plan", "servos", "servo_config", "servo_mix_rule", "adjustments", "adjustment_range", "change_plan"}, "Mixer, motor, and servo reads are implemented; mixer configuration, motor configuration, 3D motor configuration, servo configuration, servo mix rule, and adjustment range changes have confirmed typed MSP writes; motor output testing is bounded, dangerous, and confirmation-gated."),
